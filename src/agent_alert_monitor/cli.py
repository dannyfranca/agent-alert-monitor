...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--project",
        help=(
            "Project slug from config.yaml. Watchdog commands process all "
            "projects when omitted. Alert commands use the configured default project."
        ),
    )
    sub = parser.add_subparsers(dest="command", required=True)

    manual = sub.add_parser(
        "manual-alert", help="Run a local manual alert through the coordinator"
    )
    manual.add_argument("--message-id", default="manual-1")
    manual.add_argument("--text", required=True)
    manual.add_argument("--dry-run", action="store_true", default=False)
...
def _coordinator_for_config(
    ...
    ledger = (
        None
        if dry_run and args.command == "manual-alert"
